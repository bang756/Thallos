...
- (추가)
- 초점거리 계산
- 픽셀상의 너비와 실제 물체 너비의 비율은 초점거리와 촬영 거리의 비율과 같습니다. 즉, (픽셀상의 너비 / 실제 물체 너비) = (초점거리 / 촬영 거리) 입니다.
- f = ( W_pixel * D ) / W_real
- f: 초점거리, D: 촬영거리(카메라와 물체 사이의 거리)
-   ㄴ> 렌즈의 초점 거리는 고정되어 있지만, 촬영 거리에 따라 픽셀상의 크기가 변하기 때문에, 초점거리 계산 시 촬영 거리를 정확히 측정해야 합니다.
-   ㄴ> 위 계산은 간단한 모델이며, 실제 촬영 환경에서는 렌즈 왜곡, 카메라 설정 등 다양한 요인으로 인해 오차가 발생할 수 있습니다.
-   ㄴ> 더 정확한 초점거리 측정을 위해서는 카메라 캘리브레이션 과정이 필요합니다. 
### 통신 (SBC <-> 서버)
