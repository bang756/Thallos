...
# YOLO 모델 로드
model = YOLO("yolov8n.pt") #"/home/heejin/Documents/Thallos/yolov8_custom14/weights/best.pt"
model.to('cuda')

# ROI의 위쪽/아래쪽 y좌표 및 마스크 포함 임계값 설정 #777
danger_bottom, danger_top = 480, 380
warning_bottom, warning_top = 480, 320
...
while True:
    try:
        t0 = time.time()  # 전체 루프 시작 시간 #999

        # 프레임 수신
        length_buf = recvall(conn, 4) #여기서 제일 많이 시간을 할애하는것을 확인함(디버깅 코드를 통해)
        if not length_buf:
            break
        frame_len = struct.unpack('>I', length_buf)[0]
        frame_data = recvall(conn, frame_len)
        frame = cv2.imdecode(np.frombuffer(frame_data, dtype=np.uint8), cv2.IMREAD_COLOR)
        t_recv = time.time() #999
        ...
        try:
            ...
            print(f"[JSON 직렬화 에러] {e}") #999
            print("문제가 된 데이터:", danger_objects) #999
            continue
        t_serialize = time.time() #999

        #conn.sendall(response_json.encode('utf-8'))
        payload = response_json.encode('utf-8')
        conn.sendall(struct.pack('>I', len(payload)))  # 4바이트 길이
        conn.sendall(payload)
        t_send = time.time() #999
        ...
        if cv2.waitKey(1) == 27:
            break
        t_visual = time.time() #999

        # --- 요약 출력 --- #999
        print(f"[TIME] recv: {(t_recv - t0)*1000:.1f}ms | ROI: {(t_roi - t_recv)*1000:.1f}ms | YOLO: {(t_yolo - t_roi)*1000:.1f}ms | "
              f"process: {(t_process - t_yolo)*1000:.1f}ms | serialize: {(t_serialize - t_process)*1000:.1f}ms | "
              f"send: {(t_send - t_serialize)*1000:.1f}ms | visual: {(t_visual - t_send)*1000:.1f}ms")
