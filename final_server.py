...
while True:
    try:
        ...
        # 클라이언트에게 전송 (JSON 직렬화)
        try: #999
            response_json = json.dumps(danger_objects) #(JSON 직렬화)
        except Exception as e: #999
...
# Boxes 객체 안에 .xyxy, .cls, .conf 등의 여러 텐서 정보가 포함되어 있습니다.
# results.boxes = Boxes(
#     xyxy = tensor([[100.3, 200.5, 180.7, 290.1],
#                    [300.2, 220.1, 380.5, 310.3]]),
#     cls  = tensor([0, 2]),
#     conf = tensor([0.88, 0.72])
# )

# # 내부 반복시:
# for box in results.boxes:
#     box.xyxy → tensor([[x1, y1, x2, y2]])   ← (1,4) 모양
#     box.cls  → tensor([class_id])          ← (1,) 모양
#     box.conf → tensor([confidence])        ← (1,) 모양